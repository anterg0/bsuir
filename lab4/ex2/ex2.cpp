...
int main() {
    std::cout << "Цель задания: Найти минимальный элемент в каждом столбце 
...
std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    std::cout << "\nЖелаете заново ввести элементы 
массива? (y/n)";
                    char a;
                    std::cin >> a;
                    switch(a) {
            ...
            for (int i = 0; i < m; i++) {
                ...
                std::cout << "В столбце #" << column << " минимальный 
элемент: " << min << std::endl;
            }
            std::cout << "\nЖелаете повторить? (y/n)";
            char a;
            std::cin >> a;
